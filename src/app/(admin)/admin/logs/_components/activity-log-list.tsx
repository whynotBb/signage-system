...
function ActionBadge({ type }: { type: string }) {
  ...
  return (
    <Badge className={`border-0 text-xs font-medium ${variants[type] ?? 'bg-muted text-muted-foreground'}`}>
        ...
        <>
          ...
          <div className="hidden sm:block overflow-x-auto rounded-lg border border-border">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-px whitespace-nowrap">시각</TableHead>
                ...
                {data.data.map((log) => (
                  <TableRow key={log.id}>
                    ...
                    <TableCell className="whitespace-nowrap text-muted-foreground">
                      {TARGET_TYPE_LABEL[log.target_type] ?? log.target_type}
                    </TableCell>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between text-sm text-muted-foreground">
              <span>{data.count}건 중 {page * PAGE_SIZE + 1}–{Math.min((page + 1) * PAGE_SIZE, data.count)}건</span>
