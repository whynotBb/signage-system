...
export function NewsCropDialog({ open, onOpenChange, imageSrc, onCropComplete }: NewsCropDialogProps) {
  ...
  async function handleConfirm() {
    ...
    setIsCropping(true)
    try {
      const blob = await getCroppedImg(imageSrc, croppedAreaPixels, 400, 1200, 675)
      onCropComplete(blob)
      onOpenChange(false)
    } catch {
      // 크롭 실패 시 원본 사용
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="relative h-56 w-full overflow-hidden rounded-md bg-black">
          <Cropper
            image={imageSrc}
            crop={crop}
            zoom={zoom}
