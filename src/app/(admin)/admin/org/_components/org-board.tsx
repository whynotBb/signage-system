...
import { DndContext, PointerSensor, KeyboardSensor, useSensor, useSensors, DragOverlay, closestCenter, type DragEndEvent, type DragStartEvent } from "@dnd-kit/core";
import { SortableContext, sortableKeyboardCoordinates, useSortable, verticalListSortingStrategy, arrayMove } from "@dnd-kit/sortable";
import { CSS } from "@dnd-kit/utilities";
import { createClient } from "@/lib/supabase/client";
import { queryKeys } from "@/lib/supabase/query-keys";
import { useAuthStore } from "@/store/auth-store";
import { Crown, GripVertical, Plus, Pencil, Users, Trash2 } from "lucide-react";
...
async function updateDivisionOrders(items: Division[]): Promise<void> {
	const supabase = createClient();
	const updates = items.map((item, index) => ({ ...item, display_order: index + 1 }));
	const { error } = await supabase.from("divisions").upsert(updates);
	if (error) throw error;
}
		...
		<>
			...
			{!isEditor && (
				<div className="flex shrink-0 items-center gap-1 opacity-0 group-hover:opacity-100">
					<Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onEdit?.(employee)}>
...
function SortableEmployeeRow(props: EmployeeRowProps) {
	...
	return (
		<div ref={setNodeRef} style={style} className="group flex items-center gap-2 rounded-md px-2 py-1.5 hover:bg-accent/50">
			<GripVertical className="h-4 w-4 shrink-0 cursor-grab touch-none text-muted-foreground/40 group-hover:text-muted-foreground" {...attributes} {...listeners} />
...
function TeamBlockContent({ team, employees, isEditor, onAddEmployee, onEditTeam, onDeleteTeam, onEditEmployee, onDeleteEmployee }: TeamBlockProps) {
	const queryClient = useQueryClient();
	const [localEmployees, setLocalEmployees] = useState<Employee[]>(employees);

	const sensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 8 } }), useSensor(KeyboardSensor, { coordinateGetter: sortableKeyboardCoordinates }));

	// employees prop이 바뀌면 로컬 상태 동기화
	if (localEmployees.length !== employees.length || localEmployees.some((e, i) => e.id !== employees[i]?.id)) {
			...
			<div className="flex items-center gap-2 px-3 py-2">
				...
				{!isEditor && (
					<div className="flex items-center gap-1">
						<Button variant="ghost" size="sm" className="h-7 gap-1 px-2 text-xs" onClick={() => onAddEmployee?.(team.id)}>
			...
			{localEmployees.length > 0 ? (
				<div className="border-t border-border/40 px-2 py-1">
					<DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
						<SortableContext items={localEmployees.map((e) => e.id)} strategy={verticalListSortingStrategy}>
							{localEmployees.map((employee) => (
								<SortableEmployeeRow key={employee.id} employee={employee} isEditor={isEditor} onEdit={onEditEmployee} onDelete={onDeleteEmployee} />
							))}
						</SortableContext>
					</DndContext>
				</div>
			) : (
				<div className="border-t border-border/40 px-3 py-2 text-xs text-muted-foreground">직원이 없습니다</div>
			)}
		</div>
	);
}
...
function SortableTeamBlock(props: TeamBlockProps) {
	...
	return (
		<div ref={setNodeRef} style={style} className="relative">
			{!props.isEditor && (
				<div className="absolute left-0 top-0 z-10 flex h-full cursor-grab items-center px-1 touch-none" {...attributes} {...listeners}>
					<GripVertical className="h-4 w-4 text-muted-foreground/40 hover:text-muted-foreground" />
				</div>
			)}
			<div className={!props.isEditor ? "pl-5" : ""}>
				<TeamBlockContent {...props} />
			</div>
		</div>
	);
}
...
function DivisionCardContent({ division, teams, directEmployees, employees, isEditor, onAddTeam, onAddEmployee, onEditDivision, onDeleteDivision, onEditTeam, onDeleteTeam, onEditEmployee, onDeleteEmployee }: DivisionCardProps) {
	const queryClient = useQueryClient();
	const [localTeams, setLocalTeams] = useState<Team[]>(teams);
	const [localDirectEmployees, setLocalDirectEmployees] = useState<Employee[]>(directEmployees);

	if (localTeams.length !== teams.length || localTeams.some((t, i) => t.id !== teams[i]?.id)) {
		setLocalTeams(teams);
	}
	if (localDirectEmployees.length !== directEmployees.length || localDirectEmployees.some((e, i) => e.id !== directEmployees[i]?.id)) {
		setLocalDirectEmployees(directEmployees);
	}

	const teamSensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 8 } }), useSensor(KeyboardSensor, { coordinateGetter: sortableKeyboardCoordinates }));

	const directEmpSensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 8 } }), useSensor(KeyboardSensor, { coordinateGetter: sortableKeyboardCoordinates }));
	...
	return (
		<div className="rounded-lg border border-border bg-card shadow-sm" style={{ borderLeftWidth: 4, borderLeftColor: color }}>
			{/* 실 헤더 */}
			<div className="flex items-center gap-2 px-4 py-3">
				...
				{!isEditor && (
					<div className="flex items-center gap-1">
						<Button variant="ghost" size="sm" className="h-7 gap-1 px-2 text-xs" onClick={() => onAddTeam?.(division.id)}>
							<Plus className="h-3 w-3" />팀
						</Button>
						<Button variant="ghost" size="sm" className="h-7 gap-1 px-2 text-xs" onClick={() => onAddEmployee?.(division.id)}>
							<Plus className="h-3 w-3" />
							직원
						</Button>
						<Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => onEditDivision?.(division)}>
							<Pencil className="h-3 w-3" />
						</Button>
				...
				<div className="flex flex-col gap-2 px-4 pb-3">
					{localTeams.length > 0 && (
						<DndContext sensors={teamSensors} collisionDetection={closestCenter} onDragEnd={handleTeamDragEnd}>
							<SortableContext items={localTeams.map((t) => t.id)} strategy={verticalListSortingStrategy}>
								{localTeams.map((team) => {
									const teamEmployees = employees.filter((e) => e.team_id === team.id);
									return <SortableTeamBlock key={team.id} team={team} employees={teamEmployees} isEditor={isEditor} onAddEmployee={(teamId) => onAddEmployee?.(division.id, teamId)} onEditTeam={onEditTeam} onDeleteTeam={onDeleteTeam} onEditEmployee={onEditEmployee} onDeleteEmployee={onDeleteEmployee} />;
								})}
							</SortableContext>
						</DndContext>
					)}
					...
					{localDirectEmployees.length > 0 && (
						<div className="rounded-md border border-dashed border-border/60 bg-background/50">
							...
							<div className="border-t border-border/40 px-2 py-1">
								<DndContext sensors={directEmpSensors} collisionDetection={closestCenter} onDragEnd={handleDirectEmpDragEnd}>
									<SortableContext items={localDirectEmployees.map((e) => e.id)} strategy={verticalListSortingStrategy}>
...
function SortableDivisionCard(props: DivisionCardProps) {
	...
	return (
		<div ref={setNodeRef} style={style} className="relative">
			{!props.isEditor && (
				<div className="absolute left-0 top-0 z-10 flex h-full cursor-grab items-start pt-3 px-1 touch-none" {...attributes} {...listeners}>
					<GripVertical className="h-5 w-5 text-muted-foreground/40 hover:text-muted-foreground" />
				</div>
			)}
			<div className={!props.isEditor ? "pl-5" : ""}>
				<DivisionCardContent {...props} />
			</div>
		</div>
	);
}
...
function RepresentativeCard({ employee, label, isEditor, onAssign, onEdit }: { employee: Employee | null; label: string; isEditor: boolean; onAssign?: () => void; onEdit?: (employee: Employee) => void }) {
	if (!employee) {
		return (
			<div className="flex flex-1 flex-col items-center gap-3 rounded-lg border border-dashed border-border p-4 text-center">
				<div className="flex h-12 w-12 items-center justify-center rounded-full bg-muted">
					<Crown className="h-5 w-5 text-muted-foreground/50" />
				</div>
				<div>
					<p className="text-xs font-semibold text-muted-foreground">{label}</p>
					<p className="text-xs text-muted-foreground/60">미지정</p>
				</div>
				{!isEditor && (
	...
	return (
		<div className="flex flex-1 flex-col items-center gap-3 rounded-lg border border-border bg-card p-4 text-center shadow-sm">
			...
			{!isEditor && (
				<Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => onEdit?.(employee)}>
...
export function OrgBoard() {
	...
	// 실 순서 로컬 상태 (optimistic update)
	const [localDivisions, setLocalDivisions] = useState<Division[]>(divisions);

	if (localDivisions.length !== divisions.length || localDivisions.some((d, i) => d.id !== divisions[i]?.id)) {
	...
	// ── DnD ──────────────────────────────────────────────────────────────────
	const divisionSensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 8 } }), useSensor(KeyboardSensor, { coordinateGetter: sortableKeyboardCoordinates }));
	...
	const activeDivision = activeDivisionId ? localDivisions.find((d) => d.id === activeDivisionId) : null;
		...
		<div className="flex flex-col gap-6">
			...
			<div className="flex flex-col gap-2">
				<div className="flex items-center gap-1">
					...
					<span className="text-sm font-semibold text-muted-foreground">대표 · 부대표</span>
				</div>
				<div className="flex gap-3 sm:max-w-sm">
					<RepresentativeCard
			...
			{!isEditor && (
				<div className="flex items-center gap-2">
					...
					<Button variant="outline" size="sm" className="gap-1" onClick={() => openAddTeam(null)}>
						<Plus className="h-3 w-3" />독립 팀 추가
					</Button>
					<Button variant="outline" size="sm" className="gap-1" onClick={() => openAddEmployee(null, null)}>
			...
			<div className="flex flex-col gap-4">
				{localDivisions.length === 0 && independentTeams.length === 0 ? (
					...
					<>
						<DndContext sensors={divisionSensors} collisionDetection={closestCenter} onDragStart={handleDivisionDragStart} onDragEnd={handleDivisionDragEnd}>
							<SortableContext items={localDivisions.map((d) => d.id)} strategy={verticalListSortingStrategy}>
								{localDivisions.map((division) => {
									...
									const directEmployees = employees.filter((e) => e.division_id === division.id && !e.team_id && e.org_role === "member");
									return (
							...
							<DragOverlay>
								{activeDivision && (
									<div
										className="rounded-lg border border-border bg-card px-4 py-3 shadow-lg opacity-90"
										style={{
											borderLeftWidth: 4,
